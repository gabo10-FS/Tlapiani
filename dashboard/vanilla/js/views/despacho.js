...
export async function mountDespacho(root) {
      ...
      <section class="card" id="dsp-form-card">
        ...
        <form id="dsp-form" novalidate>
          ...
          <label class="field" style="margin-bottom:14px">
            ...
            <input name="transportista" required minlength="3" placeholder="Ej. Rutas del Sur S.A." />
          </label>
  ...
  const disponibles = lotes.filter(l => l.estado === 'Registrado' || l.estado === 'En Ruta');
  ...
  document.getElementById('dsp-form').addEventListener('submit', async (e) => {
    ...
    const transportista = fd.get('transportista').trim();
    const vehiculo = fd.get('vehiculo').trim();
    const lote = lotes.find(l => l.id === loteId) || { id: loteId };

    const btn = form.querySelector('button[type="submit"]');
    btn.disabled = true; btn.textContent = 'Generando hash…';
    const res = await api.despachar(loteId, transportista);
    btn.disabled = false; btn.textContent = 'Confirmar despacho & generar QR';

    // Payload que la app móvil escaneará
    const payload = JSON.stringify({ id: loteId, hash: res.hash, destino: lote.comunidad, transportista, vehiculo, t: res.despachadoEn });
    renderQR({ lote, res, payload, transportista, vehiculo });
  });
