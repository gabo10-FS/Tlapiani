...
pytest                          # full suite on in-memory SQLite (no DB needed); set TEST_MYSQL_URL to run it against a real MariaDB (see backend/README.md)
```
...
To point it at the real backend instead of its built-in mocks, `js/api.js` already has `API_BASE = 'http://127.0.0.1:8000'` and `DEMO_MODE = false` — see the block comment at the top of that file for exactly which endpoints are real vs. still mocked. The backend now also exposes centros de acopio, galería (per-comunidad photo upload, multipart), noticias, and historias (migration `0002_contenido_publico`) — the public site's Noticias/Historias sections and the map's "centros cercanos" widget are wired to these for real, and admins manage all four from the "Contenido público" screen (`js/views/contenido.js`, `#/contenido`). The one remaining gap is listing all lotes — `api.lotes()` works around that with an in-session cache rather than inventing an endpoint the backend doesn't have. Run the backend first (below), then this. **CORS**: the backend's `CORS_ORIGINS` must include `http://localhost:8099` (and `http://127.0.0.1:8099`) or the browser will block every request — both `.env` and `.env.example` already list them alongside the old `:3000` origin.
...
- **Lote ID generation is concurrency-safe by design**: `app/services/integridad_service.py::siguiente_lote_id` upserts the `lote_secuencias` table (one row per year). Two branches by dialect: MariaDB/MySQL uses `INSERT ... ON DUPLICATE KEY UPDATE ... LAST_INSERT_ID(ultimo_numero + 1)` + a follow-up `SELECT LAST_INSERT_ID()` in the same transaction (MariaDB does **not** support `RETURNING` with `ON DUPLICATE KEY UPDATE`); SQLite (tests only) uses `ON CONFLICT ... RETURNING`. Don't replace with `SELECT COUNT(*) + 1`; it races. Guarded by `tests/test_sql_dialect_compat.py`.
...
- **Time convention**: `app/core/time.py` is the single source — `ahora_utc()` (naive UTC, truncated to whole seconds) fills every model timestamp `default`/`onupdate` (never `func.now()`, which is MariaDB server-local); `app/schemas/_tiempo.py::FechaUtcZ` serializes every datetime response field as `%Y-%m-%dT%H:%M:%SZ` — the exact same format as the sello timestamp. `formatear_timestamp` in `integridad_service` is now an alias of `app/core/time.formatear_utc_z`.
...
Views: `inventario`, `mapa` (Leaflet, prioridad + centros cercanos), `despacho` (QR + hash), `transparencia` (público, no auth), `usuarios` (RF-2.1, solo Administrador), `contenido` (galería/centros de acopio/noticias/historias, solo Administrador), plus `bienvenida`/`publico` for the landing/public site. The old `js/views/galeria.js` (100% mock, in-memory `dataURL` upload, never persisted) was deleted — `contenido.js` replaces it with real multipart uploads against the backend.

### Rediseño visual (inspirado en Cruz Roja Mexicana / GoFundMe)

`css/styles.css` define `--shadow-soft`/`--shadow-lift` (por tema) usados en `.card` y en las tarjetas del sitio público (`.news-card`, `.hist-card`, `.feat-card`) para el efecto "lift" al pasar el mouse; `.btn` es ahora pill-shaped (`border-radius: 999px`) en todos lados, admin incluido, porque comparten la misma clase. `.pub-section--tint` (aplicada a `#sec-noticias` y `#sec-hacemos` en `publico.js`) crea el ritmo de bandas alternas del sitio público sin recurrir al truco de fondo a sangre completa (evita el riesgo de scroll horizontal).

La pieza nueva es el mapa "Ubica tu estado" (`#sec-estados` en `publico.js`, función `buildStateMap` en `mapCommon.js`): silueta de los 32 estados vía `dashboard/data/mx-estados.geojson` (GeoJSON público, ~180 KB, de `angelnmara/geojson`), coloreada con variables CSS (`var(--accent-emerald)`/`var(--bg-tertiary)`) para que se adapte sola al tema sin redibujar. Al hacer clic en un estado abre un panel con las comunidades ahí registradas; cada una hace `flyTo` + abre la ficha en el mapa de comunidades de abajo (`#sec-mapa`), reutilizando `renderCommunityPanel` — no son dos flujos separados. Los estados sin comunidades no muestran nada fabricado, solo un mensaje honesto.

`mapCommon.js::destroyActiveMap()` ahora limpia una lista de mapas activos (`activeCleanups`) y de callbacks de cambio de tema (`themeSwapCallbacks`) en vez de un solo mapa — necesario porque `publico.js` monta dos instancias de Leaflet a la vez (el mapa de estados y el mapa de comunidades). Si agregas un tercer mapa en cualquier vista, regístralo ahí también o quedará sin destruirse al cambiar de ruta.

**Bug de contrato corregido de paso**: `GET /api/v1/comunidades/prioridad` exigía JWT (`get_current_usuario`) aunque `publico.js` lo llama sin sesión — dejaba el sitio de visitantes literalmente roto para cualquiera sin cuenta (el `Promise.all` de `mountPublico` nunca resolvía). Se quitó esa dependencia; nombre/estado/coordenadas/score de una comunidad no son datos sensibles y ya sigue el mismo patrón público que `centros-acopio`/`noticias`/`historias`/`galería`.

### `js/api.js` — real backend for what exists, honest mocks for what doesn't

`DEMO_MODE = false` and `API_BASE = 'http://127.0.0.1:8000'` are set, but this isn't a single on/off switch: each function in `api.js` either calls the real FastAPI endpoint or falls back to `mock/data.js`, function by function, with a comment saying which and why. The real backend covers auth/login, usuarios (registrar+listar, Administrador-only), comunidades/prioridad, donaciones (registrar, despachar, historial), centros-acopio, galería (per-comunidad, multipart upload), noticias, and historias — see the block comment at the top of `api.js` for the exact endpoint list. Notably **there is no `GET` to list all lotes** — `api.lotes()` keeps an in-session cache seeded by `crearLote()`/`despachar()` results rather than inventing an endpoint the backend doesn't have. Don't "finish" this by making every function hit the network; check `backend/openapi.json` (`GET /openapi.json` on a running instance) before assuming an endpoint exists.
